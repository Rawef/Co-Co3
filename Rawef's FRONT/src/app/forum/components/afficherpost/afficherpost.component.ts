...
export class AfficherpostComponent implements OnInit {
  ...
  submitComment(postId: number): void {
    if (this.CommentForm.valid) {
      ...
      this.analyzeCommentToxicity(this.userId, description_comment);
    }
  }

  analyzeCommentToxicity(userId: number, commentText: string): void {
      ...
      (result: any) => {
        ...
        if (result.analysisResult && result.analysisResult.attributeScores && result.analysisResult.attributeScores.TOXICITY) {
          ...
          if (toxicityScore > this.toxicityThreshold) {
            console.warn(`Commentaire potentiellement toxique de l'utilisateur ${userId}:`, commentText);
            console.warn(`Score de toxicité : ${toxicityScore}`);
            console.warn(`Utilisateur ID : ${userId}`);
  
            // Si le score de toxicité est supérieur à 0.2, prendre en compte l'ID de l'utilisateur
            if (toxicityScore > 0.2) {
              console.warn(`Action supplémentaire à prendre pour l'utilisateur ID : ${userId}`);
              // Implémentez ici les actions supplémentaires à prendre pour cet utilisateur
            }
          }
        }
      },
